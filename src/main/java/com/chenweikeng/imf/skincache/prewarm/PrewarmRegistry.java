 ...
 *
 * Strategy:
 * 1. For each skull profile in the chunk, check if we have BOTH:
 *    - ProfileCache entry (UUID → texture URL + hash + texture ID path)
 *    - TextureCache entry (URL → PNG bytes on disk)
 * 2. If yes: synchronously register the texture with TextureManager on the
 *    main thread (we're already on it during chunk load). This means the
 *    GPU texture exists BEFORE the renderer ever calls getOrDefault().
 * 3. Then call lookup() to populate PlayerSkinRenderCache — since the GPU
 *    texture is already registered, the async chain completes instantly.
 * 4. If no profile cache hit: fall back to async lookup() (original behavior).
 */
public final class PrewarmRegistry {

    private static final ConcurrentHashMap<String, Set<ResolvableProfile>> chunkProfiles = new ConcurrentHashMap<>();

    private PrewarmRegistry() {}

    /**
     * Pre-warm skull profiles from a loaded chunk.
     * Called on the main thread from ClientChunkCacheMixin.
     */
    public static void prewarmChunk(String worldName, int chunkX, int chunkZ,
        ...
        for (ResolvableProfile profile : profiles) {
            ...
            try {
                ...
                SkinCacheMod.LOGGER.debug("[SkinCache] SYNC registered texture for UUID={} id={}",
                        uuid, textureId);
            } catch (Exception e) {
                SkinCacheMod.LOGGER.warn("[SkinCache] Failed to sync-register texture for UUID={}", uuid, e);
                skinRenderCache.lookup(profile);
                asyncCount++;
                continue;
            }
        ...
        if (syncCount > 0 || asyncCount > 0) {
            SkinCacheMod.LOGGER.info("[SkinCache] Pre-warmed chunk [{}, {}]: {} sync, {} async",
                    chunkX, chunkZ, syncCount, asyncCount);
        }
    }

    /**
     * Replicate SkinTextureDownloader.processLegacySkin for 32px tall legacy skins.
     * 64x64 skins pass through unchanged.
     */
