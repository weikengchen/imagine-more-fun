...
/**
 * Extracts texture URL and hash directly from a ResolvableProfile's
 * embedded properties, without any network call.
 *
 * The "textures" property in GameProfile contains a Base64-encoded JSON
 * payload with the skin URL. This is available immediately from NBT/network
 * data, even for fake/random UUIDs.
 */
public final class ProfileTextureExtractor {
    ...
    /**
     * Extract skin texture info from a ResolvableProfile.
     * Returns null if the profile has no embedded texture data.
     * Results are cached by texture URL for fast repeat lookups.
     */
    public static SkinInfo extract(ResolvableProfile profile) {
        GameProfile gameProfile = profile.partialProfile();

        Property texturesProp = Iterables.getFirst(
                gameProfile.properties().get("textures"), null);
        ...
        try {
            ...
            String textureIdPath = "skins/" + Hashing.sha1()
                    .hashUnencodedChars(hash).toString();
