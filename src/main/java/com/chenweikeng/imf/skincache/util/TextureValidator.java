...
/**
 * Validates that a downloaded skin texture is a well-formed PNG with
 * correct Minecraft skin dimensions before it gets written to the cache.
 *
 * Checks performed:
 *   1. Non-null, non-empty byte array
 *   2. PNG magic bytes (89 50 4E 47 0D 0A 1A 0A)
 *   3. Decodable as a valid image via ImageIO
 *   4. Dimensions are either 64x64 (modern) or 64x32 (legacy)
 */
public final class TextureValidator {
    ...
    /**
     * Full validation pipeline. Returns true only if ALL checks pass.
     * Decodes the PNG only once to check both decodability and dimensions.
     */
    public static boolean isValid(byte[] data) {
        ...
        try {
            ...
            SkinCacheMod.LOGGER.warn("[SkinCache] Validation failed: PNG could not be decoded (corrupted?)");
            return false;
        }
        if (img == null) {
            SkinCacheMod.LOGGER.warn("[SkinCache] Validation failed: PNG could not be decoded (corrupted?)");
        ...
        SkinCacheMod.LOGGER.warn("[SkinCache] Validation failed: bad dimensions {}x{} (expected 64x64 or 64x32)", w, h);
        return false;
    }
